...
object GameHelper {
  ...
  def startGame(players: List[Player]): Game = {
    ...
    var landsText = scala.io.Source.fromFile("src/main/resources/GameData/lands_ru.txt").mkString
    //println(landsText)
  ...
  def showGameState(g: Game, curr: String):String =
    {
        val infoText = g.State match
        {
            ...
            case GameState.BeginStep => if g.Config.isManualRollMode then "start round: choose one number [1..6]" else "write [game roll]"
            case GameState.CanBuy => s"you can buy ${g.currCell.title} or auction, write [game b] or [game a]"
            case GameState.Auction => "do you want bid? [y n]"
            case GameState.Trade => s"player ${g.currTradeBox.from.id} wants trade, write [game y] or [game n]"
            ...
            case GameState.EndStep => "press any key to  finish round or #p to show methods trace"
  ...
  def processCommand(g: Game, cmd: String, curr: String):Unit =
    {
        g.State match
        {
            ...
            case GameState.CantPay | GameState.NeedPay => PlayerManager.pay(g)

            case GameState.RandomCell => g.finishStep("")
            case GameState.MoveToCell => PlayerStepsManager.moveAfterRandom(g)

            case GameState.EndStep => if (cmd == "p") printMethodTrace(g) else g.finishGameRound()
            case _ => "default"
        }
    }
