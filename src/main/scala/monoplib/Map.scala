...
class Map(var g: Game) {
    ...
    def CellsByUserByGroup(pid: Int, group: Int) = g.Cells.filter( c=> c.owner == pid && c.group == group )
    def CellsByUserByType(pid: Int, ctype: Int) = g.Cells.filter ( c=> c.owner == pid && c.ctype == ctype )

    def GetHotelsAndHouses(pid: Int): HotelsHouses = {
        val cc = CellsByUserByType(pid, 1)
        var houses = cc.filter ( c =>c.housesCount > 0 && c.housesCount < 5 ).foldLeft(0) (_ + _.housesCount )
        var hotels = cc.filter (_.housesCount == 5).foldLeft(0) (_ + _.housesCount )
        return HotelsHouses(hotels, houses)
    }

    def MonopGroupsByUser(pid: Int) = CellsByUserByType(pid, 1).filter(_.isMonopoly)
        .groupBy( _.group ).filter(x=> x._2.forall( _.IsActive ) )
    ...
    def UpdateCellsGroupInfo() ={
        var groups = g.Cells.filter(x => x.Land() && x.owner != -1 )
            .groupBy(x=> (x.group, x.owner) );
        groups.foreach ( group =>
            group._2.foreach( gcell =>
                gcell.ownerGroupCount = group._2.size)
